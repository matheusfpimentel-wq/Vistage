// Gera o PNG-fonte 1024×1024 do ícone do app desktop (logo v6: "V" de dois
// traços separados dentro do anel pontilhado, fundo navy com cantos
// arredondados e transparência fora do squircle). Sem dependências externas:
// encoder PNG mínimo (zlib do Node + CRC32), mesma geometria do ícone mobile.
//
// Em seguida rode `tauri icon` apontando para o arquivo gerado para produzir
// .icns/.ico/PNGs/Square*Logo. Uso: node scripts/generate-icon-source.mjs
...
function iconPng(N){
  ...
  const rad = 116*s;                          // cantos arredondados (rx do svg mobile)

  // dentro do squircle (rounded-rect)? distância ao retângulo interno.
  function inside(x,y){
    const dx=Math.max(rad-x, x-(N-rad), 0);
    const dy=Math.max(rad-y, y-(N-rad), 0);
  ...
  for(let y=0;y<N;y++){
    raw[p++]=0;
    for(let x=0;x<N;x++){
      let r=0,g=0,b=0,inN=0,tot=0;
      for(const sy of SUB)for(const sx of SUB){
        tot++;
        if(inside(x+sx,y+sy)){const c=shade(x+sx,y+sy);r+=c[0];g+=c[1];b+=c[2];inN++;}
      }
      if(inN>0){raw[p++]=Math.round(r/inN);raw[p++]=Math.round(g/inN);raw[p++]=Math.round(b/inN);}
      else{raw[p++]=0;raw[p++]=0;raw[p++]=0;}
      raw[p++]=Math.round((inN/tot)*255);
    }
  }
...
const out = new URL("../icons/app-icon-source.png", import.meta.url);
writeFileSync(out, iconPng(1024));
console.log("app-icon-source.png (1024) gerado");
